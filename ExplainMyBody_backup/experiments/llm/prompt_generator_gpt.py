...
def create_fitness_prompt(analysis_result: Union[dict, "BodyAnalysisResult"]) -> Tuple[str, str]:
    """
    체형 분석 결과를 바탕으로 운동/식단 추천 프롬프트 생성

    Args:
        analysis_result: BodyAnalysisResult Pydantic 모델 또는 dict
    ...
    stage12 = data["stage1_2"]
    muscle_seg = json.dumps(data["muscle_seg"], ensure_ascii=False, indent=2)
    fat_seg_data = data.get("fat_seg")
    fat_seg = json.dumps(fat_seg_data, ensure_ascii=False, indent=2) if fat_seg_data else "정보 없음"
...
# 당신은 전문 피트니스 트레이너이자 스포츠 과학 기반 코치입니다.
...
# ---
# 출력은 반드시 아래 스키마를 따르는 JSON만 출력하세요.
# 모든 날짜는 동일한 필드 구조를 유지해야 합니다.
# 값이 없는 경우 필드를 제거하지 말고 null 또는 빈 배열([])로 채우세요.
# 설명 텍스트나 주석은 절대 출력하지 마세요.
...
#       "skip_reason": null,
#       "exercises_done": [
#         {"name": "푸시업", "sets": 3, "reps": 10, "note": "마지막 세트 힘들었음"},
#         {"name": "스쿼트", "sets": 3, "reps": 15, "note": null}
#       ],
#       "cardio": {"type": "러닝", "duration_minutes": 20, "note": "숨참"},
#       "difficulty_rating": 7,
#       "energy_level": 6,
#       "user_note": "컨디션 좋았음"
#     },
