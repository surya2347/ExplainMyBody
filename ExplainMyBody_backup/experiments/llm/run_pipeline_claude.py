...
from ollama_client import OllamaClient, LLMJsonParser
from claude_client import ClaudeClient
from openai_client import OpenAIClient
from prompt_generator_claude import create_fitness_prompt
...
def generate_recommendations(analysis_result: BodyAnalysisResult, client: OllamaClient) -> Any | None:
    """
    LLM 추천 생성 (raw 반환)
    ...
    raw_response = client.generate_chat(system_prompt, user_prompt)
    
    # 그대로 반환하여 파일 저장 시 형태 제한 없음
...
def run_pipeline(profile: InBodyProfile, client: OllamaClient, verbose: bool = True) -> PipelineResult:
    ...
    if recommendations and verbose:
        print("  Raw LLM output captured (length: " + str(len(str(recommendations))) + ")")
    elif verbose:
        print("  [Warning] Failed to get LLM output")
...
def main():
    ...
    if args.all:
        ...
        print()
        print("Usage:")
        print("  python run_pipeline_claude.py --profile-id 1")
        print("  python run_pipeline_claude.py --all")
